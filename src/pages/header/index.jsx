...
function Header() {
  const { page, windowSize, setValidateAns, setPage } = useContext(QuizContext);
  console.log(windowSize);
  const nav = useNavigate()
  return (
    <div className='flex flex-col items-center'>
      <div className="flex items-center w-full lg:px-6 p-2">
        {page <= 5 && <div className="lg:text-2xl md:text-xl bg-white px-2 rounded-md text-indigo-900 font-bold flex items-center">
          <svg
        ...
        {page <= 5 &&<p className="flex-1 text-center font-bold lg:text-2xl md:text-2xl">
          Fantasy Quiz #156
        </p>}
        <button type="button" className="bg-white rounded-full p-3"
         onClick={() => {nav('/', { replace: true}); setPage(1); setValidateAns(0)}}>
          <svg
            className='w-4 h-4'
            viewBox="0 0 18 18"
            fill="none"
      ...
      {page <= 5 && windowSize <= 320 && (
        <div className="flex items-center w-[90%] py-2">
          <div className="h-3 w-[100%] bg-gray-500 rounded-md">
            <div
