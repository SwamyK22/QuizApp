import React from 'react';

function FirstPage() {
  return (
    <div className="bg-slate-200 h-screen flex flex-col">
      <div className="flex py-4 px-8 items-center">
        <div className="text-2xl bg-white px-2 rounded-md text-indigo-900 font-bold flex items-center">
          <svg
            className="mx-1"
            width="23"
            height="23"
            viewBox="0 0 23 23"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            xlink="http://www.w3.org/1999/xlink"
          >
            <rect
              x="0.727295"
              y="0.363647"
              width="22.2727"
              height="22.2727"
              fill="url(#pattern0)"
            />
            <defs>
              <pattern
                id="pattern0"
                patternContentUnits="objectBoundingBox"
                width="1"
                height="1"
              >
                <use
                  href="#image0_15_59"
                  transform="translate(-0.00818182) scale(0.02)"
                />
              </pattern>
              <image
                id="image0_15_59"
                width="169"
                height="50"
                href="data:image/png;base64,"
              />
            </defs>
          </svg>
          200
        </div>
        <p className="flex-1 text-center font-bold text-2xl">
          Fantasy Quiz #156
        </p>
        <button type="button" className="bg-white rounded-full p-3">
          <svg
            width="18"
            height="18"
            viewBox="0 0 18 18"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M10.9582 8.99996L16.9304 3.04163C17.192 2.7801 17.3389 2.42538 17.3389 2.05552C17.3389 1.68566 17.192 1.33094 16.9304 1.06941C16.6689 0.807877 16.3142 0.66095 15.9443 0.66095C15.5745 0.66095 15.2198 0.807877 14.9582 1.06941L8.99989 7.04163L3.04156 1.06941C2.78003 0.807877 2.42531 0.66095 2.05545 0.66095C1.68559 0.66095 1.33087 0.807877 1.06934 1.06941C0.807806 1.33094 0.660879 1.68566 0.660879 2.05552C0.660879 2.42538 0.807806 2.7801 1.06934 3.04163L7.04156 8.99996L1.06934 14.9583C0.939161 15.0874 0.835836 15.241 0.765324 15.4103C0.694812 15.5795 0.658508 15.7611 0.658508 15.9444C0.658508 16.1278 0.694812 16.3093 0.765324 16.4785C0.835836 16.6478 0.939161 16.8014 1.06934 16.9305C1.19845 17.0607 1.35207 17.164 1.52132 17.2345C1.69056 17.305 1.8721 17.3414 2.05545 17.3414C2.2388 17.3414 2.42034 17.305 2.58958 17.2345C2.75883 17.164 2.91245 17.0607 3.04156 16.9305L8.99989 10.9583L14.9582 16.9305C15.0873 17.0607 15.241 17.164 15.4102 17.2345C15.5795 17.305 15.761 17.3414 15.9443 17.3414C16.1277 17.3414 16.3092 17.305 16.4785 17.2345C16.6477 17.164 16.8013 17.0607 16.9304 16.9305C17.0606 16.8014 17.164 16.6478 17.2345 16.4785C17.305 16.3093 17.3413 16.1278 17.3413 15.9444C17.3413 15.7611 17.305 15.5795 17.2345 15.4103C17.164 15.241 17.0606 15.0874 16.9304 14.9583L10.9582 8.99996Z"
              fill="black"
            />
          </svg>
        </button>
      </div>
      <p className="text-center text-indigo-900 font-bold py-4 text-3xl">
        PREDICT THE TOP LOSER(for tomorrow) across these indices
      </p>
      <div className="flex flex-col items-center justify-evenly flex-1">
        <div className="px-4 py-5 bg-white w-1/3 rounded-xl flex items-center justify-around">
          <p
            type="button"
            className="bg-slate-200 px-3 py-1 rounded-full font-bold text-xl"
          >
            A
          </p>
          <p className="text-2xl ">
            <span className="font-bold">NIFTY50 </span> $17,356,{' '}
            <span className="text-red-500">-0.315%</span>
          </p>
        </div>
        <div className="px-4 py-5 bg-white w-1/3 rounded-xl flex items-center justify-around">
          <p
            type="button"
            className="bg-slate-200 px-3 py-1 rounded-full font-bold text-xl"
          >
            B
          </p>
          <p className="text-2xl ">
            <span className="font-bold">NIFTY50 </span> $56,226,{' '}
            <span className="text-red-500">-0.315%</span>
          </p>
        </div>
        <div className="px-4 py-5 bg-white w-1/3 rounded-xl flex items-center justify-around">
          <p
            type="button"
            className="bg-slate-200 px-3 py-1 rounded-full font-bold text-xl"
          >
            C
          </p>
          <p className="text-2xl ">
            <span className="font-bold">NIFTBANK </span> $17,356,{' '}
            <span className="text-green-500">+2.12%</span>
          </p>
        </div>
      </div>
      <div className="flex justify-around items-center py-5 bg-slate-50">
        <div className="flex items-center">
          <div className="h-2 w-60 bg-gray-500 rounded-md float-left">
            <div className="bg-green-500 py-1 rounded-md w-10"></div>
          </div>
          <span className="px-2">1/5</span>
        </div>
        <button
          type="button"
          className="inline-flex justify-center rounded-xl border border-transparent bg-green-600 py-4 px-20 text-lg font-medium text-white shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
        >
          CONTINUE
        </button>
      </div>
    </div>
  );
}

export default FirstPage;
